...
class Scribd extends Shortcode {
	...
	public static function get_shortcode_ui_args() {
		return array(
			'label'          => esc_html__( 'Scribd', 'uams_shortcodes' ),
			'listItemImage'  => '<img src="' . esc_url( UAMS_SHORTCAKES_URL_ROOT . 'assets/images/svg/icon-scribd.svg' ) . '" />',
			'attrs'          => array(
				array(
					'label'        => esc_html__( 'URL', 'uams_shortcodes' ),
					'attr'         => 'url',
					'type'         => 'text',
					'description'  => esc_html__( 'Full URL to the Scribd document', 'uams_shortcodes' ),
				),
			),
		);
	}
	...
	public static function callback( $attrs, $content = '' ) {
		...
		if ( ! preg_match( '#(http|https)://www\.scribd\.com/doc(ument)?/?(\d)+/#', $attrs['url'], $needle ) ) {
			if ( current_user_can( 'edit_posts' ) ) {
				...
				return '<div class="uams-shortcodes-error"><p>' . sprintf( esc_html__( 'Invalid Scribd URL: %s', 'uams_shortcodes' ), esc_url( $attrs['url'] ) ) . '</p></div>';
			} else {
				return '';
			}
		}
		...
		$out = '<iframe class="scribd_iframe_embed uams-shortcodes-responsive" src="';
		$out .= esc_url( $url );
