...
class GoogleDocs extends Shortcode {
	...
	public static function get_shortcode_ui_args() {
		return array(
			'label'          => esc_html__( 'Google Docs', 'uams_shortcodes' ),
			'listItemImage'  => '<img src="' . esc_url( UAMS_SHORTCAKES_URL_ROOT . 'assets/images/svg/icon-googledocs.svg' ) . '" />',
			'attrs'          => array(
				array(
					'label'        => esc_html__( 'URL', 'uams_shortcodes' ),
					'attr'         => 'url',
					'type'         => 'text',
					'description'  => esc_html__( 'Full document URL', 'uams_shortcodes' ),
				),
				array(
					'label'        => esc_html__( 'Height', 'uams_shortcodes' ),
					'attr'         => 'height',
					'type'         => 'number',
					'description'  => esc_html__( 'Pixel height of the iframe.', 'uams_shortcodes' ),
				),
				array(
					'label'        => esc_html__( 'Width', 'uams_shortcodes' ),
					'attr'         => 'width',
					'type'         => 'number',
					'description'  => esc_html__( 'Pixel width of the iframe.', 'uams_shortcodes' ),
				),
				array(
					'label'        => esc_html__( 'Disable Responsiveness', 'uams_shortcodes' ),
					'attr'         => 'disableresponsiveness',
					'type'         => 'checkbox',
					'description'  => esc_html__( 'By default, height/width ratio of the embedded document will be maintained regardless of container width. Check this to keep constant height/width.', 'uams_shortcodes' ),
				),

				/* Options specific to "spreadsheet" document type */
				array(
					'label'        => esc_html__( 'Display spreadsheet header rows?', 'uams_shortcodes' ),
					'attr'         => 'headers',
					'type'         => 'checkbox',
				),
				...
				array(
					'label'        => esc_html__( 'Autostart?', 'uams_shortcodes' ),
					'attr'         => 'start',
					'type'         => 'checkbox',
				),
				array(
					'label'        => esc_html__( 'Loop?', 'uams_shortcodes' ),
					'attr'         => 'loop',
					'type'         => 'checkbox',
				),
				array(
					'label'        => esc_html__( 'Delay between slides (ms)', 'uams_shortcodes' ),
					'attr'         => 'delayms',
					'type'         => 'number',
					'default'      => 3000,
				),
	...
	public static function callback( $attrs, $content = '' ) {
		...
		$iframe_classes = "uams-shortcodes-googledocs-{$type}" .
			( empty( $attrs['disableresponsiveness'] ) ? ' uams-shortcodes-responsive' : '' );
		$url = '';
		$inner_content = '';
