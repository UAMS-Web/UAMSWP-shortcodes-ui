...
class Vimeo extends Shortcode {
	...
	public static function get_shortcode_ui_args() {
		return array(
			'label'          => esc_html__( 'Vimeo', 'uams_shortcodes' ),
			'listItemImage'  => '<img src="' . esc_url( UAMS_SHORTCAKES_URL_ROOT . 'assets/images/svg/icon-vimeo.svg' ) . '" />',
			'attrs'          => array(
				array(
					'label'        => esc_html__( 'URL', 'uams_shortcodes' ),
					'attr'         => 'url',
					'type'         => 'text',
					'description'  => esc_html__( 'Full Vimeo URL', 'uams_shortcodes' ),
				),
			),
		);
	}
	...
	public static function callback( $attrs, $content = '' ) {
		...
		return sprintf( '<div class="nc-video-player" role="region" aria-label="video" tabindex=-1><div class="tube-wrapper"><iframe class="uams-shortcodes-responsive" width="500" height="281" src="%s" frameborder="0" allowfullscreen></iframe></div></div>git', esc_url( $embed_url ) );
	}
